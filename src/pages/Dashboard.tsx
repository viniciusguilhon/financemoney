  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2 md:gap-3">
        ...
        <InfoCard icon={<CreditCard className="w-4 h-4 text-warning" />} label="Fatura Cartões" value={fmt(totalUsadoCartoes)} accent="warning" extra={totalLimiteCartoes > 0 ? `${((totalUsadoCartoes/totalLimiteCartoes)*100).toFixed(0)}% limite` : undefined} />
        <InfoCard icon={<Target className="w-4 h-4 text-primary" />} label="Investimentos" value={fmt(totalInvestido)} accent="primary" extra={`Ret: ${rentabilidade}%`} />
                  ...
                  <div key={c.id} className="space-y-1.5">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        {imgSrc ? (
                          <img src={imgSrc} alt={c.nome} className="w-8 h-5 rounded object-cover flex-shrink-0" />
                        ) : (
                          <div className="w-8 h-5 rounded flex items-center justify-center text-[7px] font-bold text-white" style={{ backgroundColor: c.cor || "hsl(215, 50%, 18%)" }}>
...
const HeroKpi = ({ label, value, icon, trend, detail, gradient }: {
  ...
  <div className={`rounded-2xl p-3 md:p-4 shadow-elevated relative overflow-hidden bg-gradient-to-br ${gradient}`}>
    ...
    <div className="relative z-[1]">
      ...
      <p className="text-base md:text-2xl font-display font-bold text-white truncate">{value}</p>
...
const InfoCard = ({ icon, label, value, accent, extra }: {
  ...
  <div className="bg-card rounded-xl p-2.5 md:p-3 shadow-card border border-border overflow-hidden">
    <div className="flex items-center gap-1.5 mb-1">
      {icon}
      <p className="text-[9px] md:text-[10px] text-muted-foreground truncate">{label}</p>
    </div>
    <p className={`text-xs md:text-sm font-bold text-${accent} truncate`}>{value}</p>
